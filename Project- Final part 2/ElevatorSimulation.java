...
public class ElevatorSimulation extends JFrame {
    ...
    void initTimers() {
        ...
        randomTimer = new Timer(3000, e -> {
            if (randomMode && rng.nextDouble() < 0.3) {
    ...
    void tick() {
        ...
        if (doorOpening) {
            ...
            // Board passengers while doors are open
            boardPassengers();
            // Check for exiting passengers
            checkPassengerExit();
            if (doorWait-- <= 0) {
                doorsOpen = false;
                doorClosing = true;
        ...
        if (moving) {
            ...
            if (Math.abs(cabY - ty) < speed) {
                ...
                // Record wait times for passengers boarding here
                for (Passenger p : passengers) {
                    if (p.startFloor == currentFloor && !p.inElevator && p.waitStart > 0) {
                        int wait = (int)((System.currentTimeMillis() - p.waitStart) / 1000);
                        waitTimes.add(wait);
                    }
                }
                updateStats();
    ...
    void spawnRandomPassenger() {
        ...
        int dest;
        do { dest = rng.nextInt(NUM_FLOORS); } while (dest == start);
    ...
    void showAddPassengerDialog() {
        ...
        JLabel startLabel = new JLabel("From Floor:");
        startLabel.setForeground(C_TEXT);
        startLabel.setFont(new Font("Consolas", Font.BOLD, 12));
        panel.add(startLabel);
        
        JComboBox<String> startCombo = new JComboBox<>();
        for (int i = 0; i < NUM_FLOORS; i++) {
            startCombo.addItem(i == 0 ? "G (Ground)" : "Floor " + i);
        }
        startCombo.setBackground(Color.WHITE);
        startCombo.setForeground(Color.BLACK);
        startCombo.setFont(new Font("Consolas", Font.PLAIN, 12));
        panel.add(startCombo);
        
        JLabel destLabel = new JLabel("To Floor:");
        destLabel.setForeground(C_TEXT);
        destLabel.setFont(new Font("Consolas", Font.BOLD, 12));
        panel.add(destLabel);
        
        JComboBox<String> destCombo = new JComboBox<>();
        for (int i = 0; i < NUM_FLOORS; i++) {
            destCombo.addItem(i == 0 ? "G (Ground)" : "Floor " + i);
        }
        destCombo.setBackground(Color.WHITE);
        destCombo.setForeground(Color.BLACK);
        destCombo.setFont(new Font("Consolas", Font.PLAIN, 12));
        panel.add(destCombo);
        
        JLabel infoLabel = new JLabel("Select different floors");
        infoLabel.setForeground(new Color(150, 150, 150));
        infoLabel.setFont(new Font("Consolas", Font.ITALIC, 10));
        panel.add(infoLabel);
        
        JPanel buttonPanel = new JPanel();
        buttonPanel.setBackground(C_BG);
        
        JButton addButton = new JButton("ADD PASSENGER");
        styleButton(addButton, C_BTN_IDLE);
        
        JButton cancelButton = new JButton("CANCEL");
        styleButton(cancelButton, C_BTN_IDLE);
        ...
        addButton.addActionListener(e -> {
            ...
            if (start == dest) {
                JOptionPane.showMessageDialog(dialog, 
                    "Start and destination floors must be different!", 
                    "Invalid Selection", 
